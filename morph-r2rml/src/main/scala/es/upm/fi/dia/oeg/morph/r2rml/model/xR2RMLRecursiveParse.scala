...
class xR2RMLRecursiveParse(termType: Option[String], parseType: Option[String], recursive_parse: xR2RMLRecursiveParse, val datatype: Option[String], val languageTag: Option[String]) {
  val logger = Logger.getLogger(this.getClass().getName());

  var parset = this.getParseType.get
  var ter = this.getTermType

  // checking the the property parseType 
  if (parset != null) {
    if (!parset.equals(Constants.R2RML_LITERAL_URI) && !parset.equals(xR2RML_Constants.xR2RML_RRXLISTORMAP_URI)) {
      throw new Exception("Illegal parsetype value in parse \n " + parset + "  is not a correct value");
    }
  }
  ...
  if (parset.equals(xR2RML_Constants.xR2RML_RRXLISTORMAP_URI)) {
    if (!ter.equals(xR2RML_Constants.xR2RML_RDFALT_URI) &&
      !ter.equals(xR2RML_Constants.xR2RML_RDFBAG_URI) &&
      !ter.equals(xR2RML_Constants.xR2RML_RDFLIST_URI) &&
      !ter.equals(xR2RML_Constants.xR2RML_RDFSEQ_URI)) {
      throw new Exception(ter + " is not a correct value for parseType " + xR2RML_Constants.xR2RML_RRXLISTORMAP_URI);
    }
  }
