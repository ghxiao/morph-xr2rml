...
class SQLJoinTable(
    val joinSource: SQLLogicalTable,
    val joinType: String,
    var onExpression: ZExpression)
        extends ZFromItem {
    ...
    def addOnExpression(onExp2: ZExpression) = {
        ...
        this.onExpression = MorphSQLUtility.combineExpresions(expressionsList, Constants.SQL_LOGICAL_OPERATOR_AND);
    }
