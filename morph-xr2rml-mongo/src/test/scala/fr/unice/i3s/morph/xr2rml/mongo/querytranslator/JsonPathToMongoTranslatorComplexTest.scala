...
class JsonPathToMongoTranslatorComplexTest {
    ...
    @Test def test_Complex() {
        ...
        var cond = new AbstractConditionEquals(jpExpr, "val")
    ...
    @Test def test_iterator() {
        ...
        var jpExpr = """$.p"""
        var query = JsonPathToMongoTranslator.trans(new AbstractConditionEquals(jpExpr, new Integer(1)), Some("$.ff.*"))
        println(query.toString)
        assertEquals(cleanString("'ff': {$elemMatch: {'p': {$eq: 1}}}"), cleanString(query.toString))

        jpExpr = """$.p[5].*.r"""
        query = JsonPathToMongoTranslator.trans(new AbstractConditionNotNull(jpExpr), Some("$.ff.tt.*"))
        println(query.toString)
