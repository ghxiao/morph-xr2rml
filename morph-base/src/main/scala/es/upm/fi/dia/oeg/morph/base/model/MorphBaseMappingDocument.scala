...
abstract class MorphBaseMappingDocument(val classMappings: Iterable[MorphBaseClassMapping]) {
  ...
  def buildMetaData(connection: Connection, databaseName: String, databaseType: String);
  ...
  def getClassMappingByPropertyURIs(propertyURIs: Iterable[String]): Iterable[MorphBaseClassMapping] = {
  ...
  def getPropertyMappingsByPropertyURI(propertyURI: String): Iterable[MorphBasePropertyMapping] = {
