...
class SQLQuery extends ZQuery with IQuery {
        ...
        orderByExp match {
          case orderByConstant: ZConstant => {
            ...
            if (orderByValue.startsWith(Constants.PREFIX_VAR)) {
              ...
              val orderByConstant2 = MorphSQLConstant.apply(orderByValue2, orderByConstant.getType(), this.databaseType, null);
      ...
      fromItem match {
        case joinTable: SQLJoinTable => {
          ...
          joinSource match {
            ...
            case _ => {
              logger.info("joinSource not SQLQuery!");
            }
  ...
  def pushProjectionsDown(pushedProjections: List[ZSelectItem], mapInnerAliasSelectItem: Map[String, ZSelectItem]) {
  ...
  def pushExpDown(pushedExp: ZExp, mapInnerAliasSelectItem: Map[String, ZSelectItem]): ZExp = {
...
object SQLQuery {
  ...
  def buildMapAliasSelectItemAux(prefix: String, innerSelectItems: Iterable[ZSelectItem]): Map[String, ZSelectItem] = {
  ...
  def create(selectItems: List[ZSelectItem], leftTable: SQLLogicalTable, rightTable: SQLLogicalTable, joinType: String, oldJoinExpression: ZExpression, databasetype: String): SQLQuery = {
    ...
    if (proceed) {
      ...
      rightTable match {
        case rightTableSQLQuery: SQLQuery => {
          ...
          val newJoinExpression = result.pushExpDown(oldJoinExpression, mapAliasSelectItems).asInstanceOf[ZExpression];
          val newWhereExpression = result.pushExpDown(rightTableWhere, mapAliasSelectItems).asInstanceOf[ZExpression];

          val logicalTableAlias = rightTable.getAlias();
          val rightTableFromItems = rightTableSQLQuery.getFrom();

          val joinTables = SQLQuery.generateJoinTables(rightTableFromItems, addedTableAlias, joinType, newJoinExpression, newWhereExpression, logicalTableAlias);
  ...
  def create(selectItems: Iterable[ZSelectItem], sqlLogicalTables: Iterable[_ <: SQLLogicalTable], whereExpression: ZExpression, databasetype: String): SQLQuery = {
  ...
  private def generateJoinTables(fromItems: Iterable[ZFromItem], addedTableAlias: List[String], joinType: String, joinExpression: ZExpression, whereExpression: ZExpression, logicalTableAlias: String): Iterable[SQLJoinTable] = {
  ...
  private def generateJoinTablesAux(fromItems: Iterable[ZFromItem], pAddedTableAlias: List[String], joinType: String, joinExpression: ZExpression, whereExpression: ZExpression, logicalTableAlias: String, pAddedExpressions: Set[ZExp]): Iterable[SQLJoinTable] = {
        ...
        case sqlFromItem: SQLFromItem => {
          ...
          if (tableType == Constants.LogicalTableType.TABLE_NAME) {
            ...
            val joinTableAux = if (relevantJoinExpressions.isEmpty() && relevantWhereExpression.isEmpty()) {
              new SQLJoinTable(rightTableLogicalTable, joinType, Constants.SQL_EXPRESSION_TRUE);
            } else {
  ...
  def createQuery(mainTable: SQLLogicalTable, joinTables: Iterable[SQLJoinTable], selectItems: List[ZSelectItem], whereCondition: ZExpression, databaseType: String): SQLQuery = {
    ...
    val result = mainTable match {
      case mainTableSQLQuery: SQLQuery => {
        ...
        val newWhere = MorphSQLUtility.combineExpresions(expressionsList, Constants.SQL_LOGICAL_OPERATOR_AND);
      ...
      case _ => {
        ...
        val newWhere = MorphSQLUtility.combineExpresionsJava(expressionsList, Constants.SQL_LOGICAL_OPERATOR_AND);
