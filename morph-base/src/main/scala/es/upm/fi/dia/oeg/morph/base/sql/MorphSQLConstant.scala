...
class MorphSQLConstant(v: String, typ: Int, val dbType: String, val columnType: String, val schema: String, val table: String, val column: String)
  ...
  def apply(zConstant: ZConstant): MorphSQLConstant = {
    ...
    zConstant match {
      case sqlConstant: MorphSQLConstant => {
        val result = this(zConstant.getValue(), zConstant.getType(), sqlConstant.dbType, sqlConstant.columnType);
        result
      }
      case _ => {
    ...
    val result = {
      typ match {
        case ZConstant.COLUMNNAME => {
          ...
          val resultAux = new MorphSQLConstant(v, typ, pDBType, columnType, schema, table, column);
          resultAux
        }
        case _ => {
