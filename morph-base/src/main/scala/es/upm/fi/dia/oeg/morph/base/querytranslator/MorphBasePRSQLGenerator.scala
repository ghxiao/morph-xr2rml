...
abstract class MorphBasePRSQLGenerator(md: R2RMLMappingDocument, unfolder: MorphBaseUnfolder) {
    ...
    def genPRSQLPredicate(tp: Triple, cm: R2RMLTriplesMap, alphaResult: MorphAlphaResult, betaGenerator: MorphBaseBetaGenerator, nameGenerator: NameGenerator, predicateURI: String): Iterable[ZSelectItem] = {
    ...
    def genPRSQLSubject(tp: Triple, alphaResult: MorphAlphaResult, betaGenerator: MorphBaseBetaGenerator, nameGenerator: NameGenerator, cmSubject: R2RMLTriplesMap): List[ZSelectItem] = {
        val tpSubject = tp.getSubject();
    ...
    def genPRSQLSTG(stg: Iterable[Triple], alphaResult: MorphAlphaResult, betaGenerator: MorphBaseBetaGenerator, nameGenerator: NameGenerator, cmSubject: R2RMLTriplesMap): MorphPRSQLResult = {
