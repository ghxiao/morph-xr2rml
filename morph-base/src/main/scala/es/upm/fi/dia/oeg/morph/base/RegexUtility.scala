...
object RegexUtility {
    ...
    def getTemplateColumns(tplStr: String, cleanColumn: Boolean): java.util.List[String] = {
        val columnsFromTemplate = TemplatePattern1.findAllIn(tplStr).toList;
        val result = {
            if (cleanColumn) {
    ...
    def replaceTokens(matcher: Matcher, text: String, replacements: java.util.Map[String, Object]): String = {
