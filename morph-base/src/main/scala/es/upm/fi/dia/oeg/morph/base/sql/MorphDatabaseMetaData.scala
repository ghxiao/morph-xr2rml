...
class MorphDatabaseMetaData(val conn: Connection, val dbName: String, val dbType: String, var tablesMetaData: List[MorphTableMetaData], jdbcDBMetaData: DatabaseMetaData) {
...
object MorphDatabaseMetaData {
  ...
  , databaseName: String, databaseType: String): MorphDatabaseMetaData = {
    ...
    val dbMetaData: MorphDatabaseMetaData = {
      if (conn != null) {
        try {
          ...
          new MorphDatabaseMetaData(conn, dbName, dbType, listTableMetaData, jdbcDBMetaData);
        } catch {
          case e: Exception => {
